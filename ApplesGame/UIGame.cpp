...
{
	void InitTextUI(sf::Text& text, const sf::Font& font, const std::string& string,
	...
	{
		InitTextUI(uiGame.controlPlayerText, font, "Arrow to Move, TAB to Menu, ESC to Exit",
			24, sf::Color::White, { SCREEN_WIDHT * (12.f / 15.f), 20.f });
		InitTextUI(uiGame.scorePlayerText, font, "Your score : 0",
			24, sf::Color::White, { SCREEN_WIDHT / 13.f, 20.f});

		InitTextUI(uiGame.gameOverText, font, "Game Over!",
			50, sf::Color::White, { SCREEN_WIDHT / 2.f, SCREEN_HEIGHT / 2.5f });
		InitTextUI(uiGame.gameOverScoreText, font, "Your score : 0",
